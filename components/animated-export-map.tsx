...
export function AnimatedExportMap() {
  return (
    <div className="relative min-h-[390px] overflow-hidden rounded-[28px] bg-[#F8F4EA] lg:min-h-[460px]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_68%_49%,rgba(214,170,57,.22),transparent_18%),radial-gradient(circle_at_48%_50%,rgba(11,74,43,.08),transparent_60%)]" />

      <svg viewBox="0 0 920 440" role="img" aria-label="Global export routes from India" className="relative block h-auto w-full overflow-visible">
        <defs>
          <linearGradient id="landGreen" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="#446D55" stopOpacity=".95" />
            <stop offset=".55" stopColor="#214D35" stopOpacity=".98" />
            <stop offset="1" stopColor="#123D29" stopOpacity=".96" />
          </linearGradient>
          <linearGradient id="routeGold" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0" stopColor="#F4C84A" stopOpacity=".65" />
            <stop offset=".45" stopColor="#E6AF23" />
            <stop offset="1" stopColor="#FFF0A5" />
          </linearGradient>
          <radialGradient id="originGlow">
            <stop offset="0" stopColor="#FFE88B" stopOpacity="1" />
            <stop offset=".38" stopColor="#EAB52B" stopOpacity=".5" />
          ...
          <filter id="shadow" x="-60%" y="-60%" width="220%" height="240%">
            <feDropShadow dx="0" dy="5" stdDeviation="6" floodColor="#102F22" floodOpacity=".18" />
          </filter>
        </defs>

        <g opacity=".18" stroke="#6B806F" strokeWidth=".65">
          <path d="M90 115H845"/><path d="M80 175H855"/><path d="M75 235H860"/><path d="M85 295H850"/>
        </g>

        <g fill="url(#landGreen)" stroke="#6F8A73" strokeWidth=".8">
        ...
        <g opacity=".17" fill="#F6D66F">
          <circle cx="333" cy="130" r="2"/><circle cx="300" cy="235" r="2"/><circle cx="430" cy="120" r="2"/><circle cx="560" cy="92" r="2"/><circle cx="680" cy="110" r="2"/><circle cx="760" cy="235" r="2"/><circle cx="620" cy="320" r="2"/>
        </g>

        {routes.map((route) => (
          <g key={route.id}>
            <path d={route.path} fill="none" stroke="url(#routeGold)" strokeWidth="2.2" strokeLinecap="round" strokeDasharray="4 5" className="route-line" style={{ animationDelay: route.delay }} />
            ...
            <circle cx={route.x} cy={route.y} r="8" fill="#FFF7D2" stroke="#D7A51F" strokeWidth="3" filter="url(#softGlow)" />
        ...
        <g transform="translate(620 214)">
          <circle r="62" fill="url(#originGlow)" className="origin-glow" />
          <circle r="30" fill="none" stroke="#EBC252" strokeWidth="1" opacity=".5" />
          <circle r="43" fill="none" stroke="#EBC252" strokeWidth="1" opacity=".28" />
      ...
      <div className="pointer-events-none absolute inset-x-[8%] bottom-[6%] h-12 bg-gradient-to-t from-[#F8F4EA] to-transparent" />
      ...
      <style jsx>{`
        ...
        @keyframes originPulse { 0% { transform: scale(.72); opacity: .5; } 100% { transform: scale(1.45); opacity: 0; } }
